...
Visualization of the tracker for $\alpha=0.5$.
...
However, we are feel that we have grown more comfortable and proficient with working on existing large-scale projects, working with remote connections on Linux systems, sending jobs to clusters, and managing substantial datasets. Through our interaction with DINOv2 and Segment-Anything we feel more comfortable with complex deep learning models and their possible implementations.

If we would have done it again, we would probably start with fine-tuning a existing visual model on an appropriate dataset, as we could see high precision but low accuracy in the results. The model could have a potential to achieve a higher score on accuracy. We would probably try a model with even stronger feature representations such as CLIP from OpenAI, and compare it with others. Additionally, we would try to make it truely online, and not pre-generate the embeddings before running the tracker.
